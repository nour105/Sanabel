...
// export default function Offers({ offers, lang }) {
...
//                 <h3 className="text-white text-xl font-bold mb-2">{typeof offer.title === 'object' ? offer.title[lang] || offer.title.en : offer.title}</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
         {offers.slice(0, 6).map((offer) => (
            ...
            <Link key={offer.id} href={`/${lang}/offers/${offer.slug || offer.id}`} className="group relative block overflow-hidden rounded-xl shadow-lg">
              ...
              <div className="absolute inset-0 bg-[#0000004f] bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-6">
                <h3 className="text-white text-xl font-bold mb-2">{typeof offer.title === 'object' ? offer.title[lang] || offer.title.en : offer.title}</h3>
              </div>
            </Link>
          ))}

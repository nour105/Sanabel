      ...
      <div className="px-4 py-16 space-y-10">
        ...
        <div className="lg:grid lg:grid-cols-3 lg:gap-10 flex flex-col gap-10">
          ...
          <div className="lg:col-span-2 flex flex-col gap-10">
            ...
            {/* Price Box */}
            <div className="bg-white rounded-3xl shadow p-8 flex flex-col md:flex-row md:items-center md:justify-between gap-6 order-1 lg:order-1">
              <div>
                <p className="text-sm text-gray-500">{lang === 'ar' ? 'السعر النقدي' : 'Cash Price'}</p>
            ...
            {car.specifications?.length > 0 && (
              <div className="bg-white rounded-3xl shadow p-6 order-2 lg:order-2">
                <h3 className="font-bold text-lg mb-4">{lang === 'ar' ? 'المواصفات' : 'Specifications'}</h3>
          ...
          <div className="flex flex-col gap-10">
            ...
            <div className="bg-white rounded-3xl shadow p-6 order-3 lg:order-1">
              ...
              <ul className="space-y-3 text-sm text-gray-700">
                <li className="flex justify-between"><span>{lang === 'ar' ? 'الماركة' : 'Brand'}</span><span className="font-semibold">{lang === 'ar' ? car?.brand?.ar : car?.brand?.en}</span></li>
                <li className="flex justify-between"><span>{lang === 'ar' ? 'الموديل' : 'Model'}</span><span className="font-semibold">{lang === 'ar' ? car?.name?.ar : car?.name?.en}</span></li>
                <li className="flex justify-between"><span>{lang === 'ar' ? 'السعر' : 'Price'}</span><span className="font-semibold">{car?.price} <Image src={SAR_symbol} alt="SAR" width={20} height={20} className="inline"/></span></li>
                <li className="flex justify-between"><span>{lang === 'ar' ? 'الأقساط الشهرية' : 'Monthly Installments (EMI)'}</span><span className="font-semibold">{car?.emi_monthly} <Image src={SAR_symbol} alt="SAR" width={20} height={20} className="inline"/></span></li>
                <li className="flex justify-between"><span>{lang === 'ar' ? 'سنة الصنع' : 'Year Model'}</span><span className="font-semibold">{lang === 'ar' ? car?.year_model?.ar : car?.year_model?.en}</span></li>
                <li className="flex justify-between"><span>{lang === 'ar' ? 'الضمان' : 'Warranty'}</span><span className="font-semibold">{lang === 'ar' ? car?.warranty?.ar : car?.warranty?.en}</span></li>
                <li className="flex justify-between"><span>{lang === 'ar' ? 'عدد الأسطوانات' : 'Cylinders'}</span><span className="font-semibold">{lang === 'ar' ? car?.cylinders?.ar : car?.cylinders?.en}</span></li>
